...
# todo remove when update docker 1.12+ (then we should use the original minio/minio image)
fix_resolv() {
...
# note: `docker_wait_hosts` logic in this script is moved to cmd/net.go in commit
# d4b107ad: Retry name lookup for kubernetes and docker swarm environment (#4800)
